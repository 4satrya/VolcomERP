...
WHERE a.id_adj_in_fg = '" & id_report & "'"
        ...
        ElseIf report_mark_type = "148" Then
            ...
            If id_status_reportx = "6" Then
                ...
                SELECT '" & id_purc_store & "' AS id_comp,IF(rq.is_store_purchase=1,'" & id_purc_store & "',rq.id_departement) AS id_departement, 1, rd.id_item, (pod.`value`/i.stock_convertion) AS value, 148, " + id_report + ", rd.qty_stock, NOW(),'', 1
                ...
                SELECT " + id_acc_trans + ",(SELECT id_acc_pendapatan_lain FROM tb_opt_accounting) AS `id_acc`, dep.id_main_comp,  
                SUM(rd.qty) AS `qty`,
                0 AS `debit`,
                ...
                UNION ALL
                /*total vat in*/
                SELECT " + id_acc_trans + ",o.acc_coa_vat_in AS `id_acc`, dep.id_main_comp,  SUM(rd.qty) AS `qty`,
                ...
                SELECT " + id_acc_trans + ", comp.id_acc_ap AS `id_acc`, dep.id_main_comp, SUM(rd.qty) AS `qty`, 0 AS `debit`,
                ...
                If dt_dp.Rows.Count > 0 Then 'ada DP
                    If dt_dp.Rows(0)("is_close_rec").ToString = "1" Then
                        ...
                        qjd += " UNION ALL
                                /* Total DP balik */
                                SELECT " + id_acc_trans + ", comp.id_acc_dp AS `id_acc`, 1, 0 AS `qty`,0 AS `debit`,SUM(pnd.`value`) AS `credit`, 
                                ...
                                UNION ALL
                                /* Hutang dagang balik */
                                SELECT " + id_acc_trans + ", comp.id_acc_ap AS `id_acc`, 1, 0 AS `qty`,SUM(pnd.`value`) AS `debit`,0 AS `credit`, 
                                'Receiving complete, inverting journal DP' AS note, '159', pn.id_pn, pn.number, po.report_mark_type, po.id_purc_order, po.purc_order_number
                                FROM tb_pn_det pnd
                                INNER JOIN tb_pn pn ON pn.id_pn=pnd.id_pn AND pn.id_report_status='6' AND pn.id_pay_type='1'
                                INNER JOIN tb_purc_order po ON po.id_purc_order=pnd.id_report AND (pnd.report_mark_type='139' OR pnd.report_mark_type='202')
...
WHERE copd.id_design_cop_propose='" & id_report & "';"
        ...
        ElseIf report_mark_type = "156" Or report_mark_type = "166" Then
            ...
            If id_status_reportx = "6" Then
                ...
                Dim query_completed_stock As String = "INSERT INTO tb_storage_item (id_comp,id_departement, id_storage_category,id_item, `value`, report_mark_type, id_report, storage_item_qty, storage_item_datetime, id_stock_status)
                SELECT '" & id_wh_storage & "' as id_storage,IF(rd.is_store_request=1,'" & id_purc_store & "',r.id_departement) AS id_departement, 1, dd.id_item, getAvgCost(dd.id_item), IF(r.is_for_store=1,163,154) AS rmt, r.id_item_req, dd.qty, NOW(), 1
                FROM tb_item_del d
                ...
                UNION ALL
                SELECT '" & id_wh_storage & "' as id_storage,IF(rd.is_store_request=1,'" & id_purc_store & "',r.id_departement) AS id_departement, 2, dd.id_item, getAvgCost(dd.id_item), " + report_mark_type + " AS rmt, d.id_item_del, dd.qty, NOW(), 1
                FROM tb_item_del d
                INNER JOIN tb_item_del_det dd ON dd.id_item_del = d.id_item_del
                INNER JOIN tb_item_req_det rd ON rd.id_item_req_det=dd.`id_item_req_det`
                INNER JOIN tb_item_req r ON r.id_item_req = d.id_item_req
                ...
                If FormItemDelDetail.is_for_store = "1" Then
                    ...
                    CONCAT('Expense : ',cat.item_cat,' (',c.comp_number,')') AS `note`, " + report_mark_type + " AS `rmt`, d.id_item_del, d.`number`, 1
                    FROM tb_item_del_det_alloc dd
                    INNER JOIN tb_item_del d ON d.id_item_del = dd.id_item_del
                    INNER JOIN tb_item_req r ON r.id_item_req = d.id_item_req
                    INNER JOIN tb_item i ON i.id_item = dd.id_item
        ...
        ElseIf report_mark_type = "157" Then
            ...
            If id_status_reportx = "6" Then
                ...
                If FormItemExpenseDet.CEPayLater.EditValue = True Then
                    ...
                    SELECT " + id_acc_trans + ", ed.id_acc, IF(ed.amount<0,0,ed.amount) AS `debit`, IF(ed.amount<0,-ed.amount,0) AS `credit`, ed.description, 157, e.id_item_expense, e.`number`
        ...
        ElseIf report_mark_type = "159" Then
            ...
            If id_status_reportx = "6" Then
                ...
                'main journal
                Dim qjm As String = "INSERT INTO tb_a_acc_trans(acc_trans_number, report_number, id_bill_type, id_user, date_created, acc_trans_note, id_report_status)
                VALUES('" + header_number_acc("1") + "','" + report_number + "','22','" + id_user_prepared + "', '" & date_created & "', 'Auto Posting', '6'); SELECT LAST_INSERT_ID(); "
                Dim id_acc_trans As String = execute_query(qjm, 0, True, "", "", "", "")
                increase_inc_acc("1")
...
SELECT '" & data_det.Rows(i)("id_sample_purc_budget").ToString & "' AS id_det,id_code_division FROM tb_sample_budget_pps_div WHERE id_sample_budget_pps_det='" & data_det.Rows(i)("id_sample_budget_pps_det").ToString & "' AND is_after='1';"
        ...
        ElseIf report_mark_type = "189" Then
            ...
            If id_status_reportx = "6" Then
                ...
                (
	                /* Per row */
	                SELECT '" & id_acc_trans & "' AS id_acc_trans,pnd.id_acc AS `id_acc`, cf.id_comp,0 AS `qty`,IF(SUM(pnd.value)<0,0,SUM(pnd.value)) AS `debit`,IF(SUM(pnd.value)>0,0,-SUM(pnd.value)) AS `credit`,pnd.id_currency,pnd.kurs,IF(pnd.id_currency=1,0,IF(SUM(pnd.value_bef_kurs)<0,0,SUM(pnd.value_bef_kurs))) AS `debit_valas`,IF(pnd.id_currency=1,0,IF(SUM(pnd.value_bef_kurs)>0,0,-SUM(pnd.value_bef_kurs))) AS `credit_valas`,'' AS `note`,189,pn.id_pn_fgpo, pn.number, pnd.report_mark_type, pnd.id_report, pnd.report_number, comp.comp_number
